...
async def honeypot_endpoint(
    ...
    # ALL entity types - not just the 4 basic ones
    entity_keys_with_hive_mind = ["bankAccounts", "upiIds", "phishingLinks", "phoneNumbers", "emailAddresses"]
    entity_keys_no_hive_mind = ["caseIds", "policyNumbers", "orderNumbers", "amounts", "suspiciousKeywords"]

    for key in entity_keys_with_hive_mind:
    ...
    # Handle non-hive-mind entity types
    for key in entity_keys_no_hive_mind:
        for value in extracted.get(key, []):
            if value:
                if key not in session_info["extracted_entities"]:
                    session_info["extracted_entities"][key] = []
    ...
    # === PER-TURN CALLBACK: Send callback every turn when intelligence exists ===
    has_intelligence = any([
        session_info["extracted_entities"].get("bankAccounts"),
        session_info["extracted_entities"].get("upiIds"),
        session_info["extracted_entities"].get("phishingLinks"),
        session_info["extracted_entities"].get("phoneNumbers"),
        session_info["extracted_entities"].get("emailAddresses"),
    ])
    if has_intelligence and is_scam:
        try:
            start_time = session_info["start_time"]
            if isinstance(start_time, str):
                start_time = datetime.fromisoformat(
        ...
        per_turn_notes = build_agent_notes(
            session_info.get("scam_type", "UNKNOWN"),
            session_info["extracted_entities"],
            "ENGAGED",
            session_info["message_count"],
        ...
        background_tasks.add_task(
            ...
            total_messages=total_exchanged,
            intelligence=session_info["extracted_entities"],
            agent_notes=per_turn_notes,
            engagement_duration=engagement_duration,
            conversation_metrics=session_info.get("conversation_metrics"),
        )
        logger.info(f"📞 [PER-TURN] Sending GUVI callback for session {session_id} (turn {session_info['message_count']})")
